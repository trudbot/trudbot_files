...
玩家向机器人发出的指令是一个仅涉及加法和减法的表达式，即对若干个数字进行一系列加法或减法计算。
 这些数字可以是直接给出的非负整数（数字不超过 1000），也可以是若干个骰子掷出的结果。

“掷骰子”这个动作对应的指令格式为 xdy，表示摇动 x 个 y 面的骰子（1≤x≤1000,2≤y≤1000）。当 x 为 1 时，1 可以省略。

例如指令 2d3+3-d4 的意思是：先掷出 2 个 3 面骰子（你不必考虑现实中是否存在这样的骰子），
 不妨假设结果为 1 和 3，则 2d3 的结果就是两个骰子的面值之和 4；然后计算 4 + 3，得到结果为 7；再掷出 1 个 4 面骰子，不妨假设结果为 2，
 则计算 7 - 2 得到最终结果 5。
...
void Solution(int l)
{
    ...
    if(str[l] == 'd') x = 1;
        ...
        while (l < str.size() && str[l] != 'd' && str[l] != '+' && str[l] != '-')
            x = x * 10 + str[l++] - '0';
    }
    if(str[l] == 'd')
    {
        l++;
        while(l < str.size() && str[l] != '+' && str[l] != '-')
            y = y*10 + str[l++]-'0';
        num[y] += x;
        if(flag == 1)
        {
